...
**1. Agent Loop 直接构建在 OpenAI Responses API 上。** `AgentLoop` 类封装了一个标准的工具循环：调用模型 → 收到 tool_call → 执行工具 → 将结果反馈 → 继续循环。这和我们在上一篇文章中讨论的 `while has_tool_calls(response)` 模式完全一致。

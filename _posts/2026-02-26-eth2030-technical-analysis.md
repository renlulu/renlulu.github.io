...
ETH2030 的作者 YQ 在[另一条推文](https://x.com/yq_acc/status/2026679327027523615)中总结了他 18 个月、90 亿 token、36 个代码库、$25,000+ API 成本的 AI coding 经验。其中一句话最为精辟：
...
1. **人类决定构建什么** — 架构、包边界、核心类型
2. **人类写精确的 15 步分解说明** — 不是"实现 PeerDAS"，而是"在 `pkg/das/sampling.go` 中实现 `GetCustodyGroups` 函数，输入 node ID 和 custody group count，输出该节点负责的列索引列表"
3. **AI 处理其余部分** — 代码生成、测试编写、bug 修复
4. **人类审查和集成** — 确保模块间接口一致
